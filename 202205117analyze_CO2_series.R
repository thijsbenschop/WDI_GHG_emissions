...
# September 2021
# Update May 2022 -> update PIK file and add SDG indicator official data
...
#PIK2<- as.data.table(read.csv("./Data_private/PIK/Old_versions/PRIMAP-hist_v1.1_06-Mar-2017.csv"))

setnames(PIK, "area..ISO3.", "Nation")
...
#source	scenario (PRIMAP-hist)	area (ISO3)	entity	unit	category (IPCC2006_PRIMAP)

PIK
PIK_long <- melt(PIK, id.vars = c("Nation", "scenario", "category", "entity", "unit"))
PIK_long

PIK_long <- PIK_long[scenario == "HISTCR"] # use country reported data by default
...
PIK_country_wide <- dcast(data = PIK_long %>% filter(category == "M.0.EL" & entity == "CO2"),
                          formula = Year ~ Nation ,
                          value.var = "CO2_emissions_ktCO2")
...
PIK_wide_USA[, USA_PIK := M.0.EL / 1000]
...
PIK_wide_CHN[, CHN_PIK := M.0.EL / 1000]
...
PIK_wide_RUS[, RUS_PIK := M.0.EL / 1000]
...
## Official SDG indicator UNFCCC
getwd()
SDG_annex1    <- as.data.table(read_xlsx("./Data_private./SDG_indicators/EN_ATM_GHGT_AIP (1).xlsx"))
SDG_nonannex1 <- as.data.table(read_xlsx("./Data_private./SDG_indicators/EN_ATM_GHGT_NAIP (1).xlsx"))
...
combined_sources_USA <- combined_sources_USA %>% filter(Year > 1960)
saveRDS(combined_sources_USA, "Data/data_for_plots/combined_sources_USA_2.rds")
...
combined_sources_CHN <- combined_sources_CHN %>% filter(Year > 1960)
saveRDS(combined_sources_CHN, "Data/data_for_plots/combined_sources_CHN_2.rds")
...
combined_sources_RUS <- combined_sources_RUS %>% filter(Year > 1960)
saveRDS(combined_sources_RUS, "Data/data_for_plots/combined_sources_RUS_2.rds")
