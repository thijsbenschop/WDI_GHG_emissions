...
```{r display_cur_indicators, echo=FALSE, results = 'asis'}
#class(cur_indicators) # sort by GHG and sector 
cur_indicators <- as.data.table(cur_indicators)
cur_indicators <- cur_indicators[order(GHG, Sector)]
...
plot(cur_indicators_tree, title = "Current GHG indicators in WDO by GHG and sector")

```

```{r display_cur_indicators_4, out.width = '300%', echo=FALSE, results = 'asis'}
...
<!-- Using hybrid scoring considering time and country coverage and usage statistics. -->
...
Licensing issues and data availability: indicators from IEA are not distributed under a license that allows redistribution.
...
also licensing terms and SDGs. The respective sources are UNIDO and UNFCCC. 
...
ggplot(data=combined_sources %>% filter(Year >= 1960), aes(Year)) +
  ...
  labs(x = "Year", y= "CO2 emissions (MtCO2)", title = "World", subtitle = "1960-latest datapoint")
...
ggplot(data=combined_sources_CHN %>% filter(Year >= 1960), aes(Year)) +
  ...
  #geom_line(aes(y = (CAT1 + CAT2)/1000, colour = "PIK - CAT1 + CAT2"), size = 1) +
  labs(x = "Year", y= "CO2 emissions (MtCO2)", title = "China", subtitle = "1960-latest datapoint")
...
ggplot(data=combined_sources_USA %>% filter(Year >= 1960), aes(Year)) +
  ...
  #geom_line(aes(y = (CAT1 + CAT2)/1000, colour = "PIK - CAT1 + CAT2"), size = 1) +
  labs(x = "Year", y= "CO2 emissions (MtCO2)", title = "USA", subtitle = "1960-latest datapoint") #+
...
ggplot(data=combined_sources_RUS %>% filter(Year >= 1960), aes(Year)) +
 ...
 # geom_line(aes(y = RUS_PIK, colour = "PIK - CAT0 - CAT5"), size = 1) +
  #geom_line(aes(y = (CAT1 + CAT2)/1000, colour = "PIK - CAT1 + CAT2"), size = 1) +
  labs(x = "Year", y= "CO2 emissions (MtCO2)", title = "Russian Federation", subtitle = "1960-latest datapoint")
...
setnames(CAIT_long, "value", "CO2_emissions_MtCO2")
...
plot_function <- function(cur_ISO3){
  ...
  cur_CAIT_wide <- CAIT_long %>% 
    ...
    dcast(formula = ... ~ Gas + Sector, value.var = "CO2_emissions_MtCO2") %>%
...
```{r plot_volatility, echo=FALSE, results = 'asis'}
...
```{r plot_coverage, echo=FALSE, results = 'asis'}

# Read counts of values by ind and year
...
All sources rely on the same set of primary sources for data.
...
* IEA - International Energy Agency: 
  + *URL* https://www.iea.org/data-and-statistics/data-product/greenhouse-gas-emissions-from-energy
  + *Time coverage*: data from 1960- (OECD countries), 1971- (non-OECD countries -2019 (2020 for some countries)
  + *Countries*: OECD countries, total 145 countries
  + *License*: -
  + *Notes*: Data only available after purchasing, only emissions from energy
* UN SDG Data Portal
  + *URL* https://unstats.un.org/sdgs/dataportal
  + *Time coverage*: 2000-2019 for Annex I countries, 2000-2018 for non-Annnex I countries -> 78% missing values
  + *Countries*: 43 Annex I countries and 136 non-Annex I countries
  + *License*: -
  + *Notes*: Only total GHG emissions without LULUCF, no disaggreation by gas or sector (Indicator 13.2.2)
...
### Sector disaggreagation

The following sector/categories are available (category descriptions using IPCC 1996 terminology).
...
Data on LULUCF has high uncertainty and we therefore exclude.
