...
# Updated: 08/25/2025 - Use European Forest Observatory Data for LULUCF 2000-2023 (received from David 8/22/2025 by email)
#                     - Clean up and keep only relevant parts
# Updated: 11/17/2025 - Use EDGAR 2025 GHG data version published October 2025
...
#### Load EDGAR data ####
# Version EDGAR_2025_GHG // Published October 2025
# Data downloaded from https://edgar.jrc.ec.europa.eu/dataset_ghg2025
# One dataset per GHG + total

folders <- list.dirs("./Data_private/EDGAR/2025/") # list unzipped folders
...
# Append all datasets
file_names # check order of file names
EDGAR_long <- rbind(get(file_names[1]), # EDGAR_AR5_GHG_1970_202x - total GHG AR5
                    get(file_names[2]), # EDGAR_AR5g_F-gases_1990_202x - F gases AR5
                    get(file_names[3]), # EDGAR_CH4_1970_202x - CH4
                    get(file_names[4]), # EDGAR_CO2bio_1970_202x - CO2 bio
                    #get(file_names[5]), # EDGAR_F-gases_1990_202x - F-gases, don't use gas-specific, use AR5
                    get(file_names[6]), # EDGAR_N2O_1970_202x - N2O
                    get(file_names[7])) # IEA_EDGAR_CO2_1970_202x - CO2 fossil from IEA
...
#### Load European Forest Observatory LULUCF data ####
## EFO data
EFO_data <- as.data.table(read_xlsx("Data_private/EFO_LULUCF/20250822timeseries_NGHGI.xlsx"))

# Select only data from Version 3
EFO_data <- EFO_data %>% filter(Version == "V3.0 (NGHGI 2025)")

# Prepare EFO data
EFO_data <- EFO_data %>% mutate(Series = case_when(
                                  Category == "DEFORESTATION" ~ "EN.GHG.CO2.LU.DF.MT.CE.AR5", # Carbon dioxide (CO2) net fluxes from LULUCF - Deforestation (Mt CO2e)    
                                  Category == "FOREST" ~ "EN.GHG.CO2.LU.FL.MT.CE.AR5", # Carbon dioxide (CO2) net fluxes from LULUCF - Forest Land (Mt CO2e)     
                                  Category == "LULUCF" ~ "EN.GHG.CO2.LU.MT.CE.AR5",	   # Carbon dioxide (CO2) net fluxes from LULUCF - Total excluding fires (Mt CO2e)  
                                  Category == "ORG_SOILS" ~ "EN.GHG.CO2.LU.OS.MT.CE.AR5", # Carbon dioxide (CO2) net fluxes from LULUCF - Organic Soil (Mt CO2e)
                                  Category == "OTHER" ~ "EN.GHG.CO2.LU.OL.MT.CE.AR5" # Carbon dioxide (CO2) net fluxes from LULUCF - Other Land (Mt CO2e)
                                  # What about HWP (Harvested Wood Products - not in WDI)
                                )) 
...
length(EDGAR_countries) # 225 countries
...
EFO_countries <- unique(EFO_data$ISO3)
length(EFO_countries) # 187 countries
table(WDI_countries$ISO3 %in% EFO_countries) # 183 WDI countries available in EFO
WDI_countries[!(ISO3 %in% EFO_countries)] # list of missing countries (not in EFO data)
# are these countries/territories included in other countries? -> need to combine countries?
table(EFO_countries %in% WDI_countries$ISO3)
EFO_countries[!(EFO_countries %in% WDI_countries$ISO3)] # list of missing countries (not in WDI list)

# Drop countries/regions not in WDI
EDGAR_long <- EDGAR_long[ISO3 %in% WDI_countries$ISO3]

EFO_data <- EFO_data %>% filter(ISO3 %in% WDI_countries$ISO3)
...
EDGAR_long <- EDGAR_long %>% mutate(sector_WDI = case_when(
  ...
  (GHG == "CH4" & category %in% c("4.A", "4.B", "4.C", "4.D", "4.E")) ~ "WA",
...
# World data is added as World includes aviation and shipping
# Spreadsheet downloaded from https://edgar.jrc.ec.europa.eu/report_2024
GHG_totals_by_country <- as.data.table(read_xlsx("./Data_private/EDGAR/2025/EDGAR_2025_GHG_booklet_2025.xlsx", 
...
GHG_by_sector_and_country <- as.data.table(read_xlsx("./Data_private/EDGAR/2025/EDGAR_2025_GHG_booklet_2025.xlsx", 
...
# LULUCF
LULUCF_macroregions <- as.data.table(read_xlsx("./Data_private/EDGAR/2025/EDGAR_2025_GHG_booklet_2025.xlsx", 
...
# Check if all lines labelled (gas and sector)
table(EFO_data$Category, useNA = "always") # 6 categories, only 5 indicaotrs (no HWP in WDI)

# Drop all lines with category HWP
EFO_data <- EFO_data %>% filter(Category != "HWP")

# Round all values to 4 digits
EFO_data <- EFO_data %>%
  mutate(value = round(CFluxes_yr, 4))

# Check that there are no NA values for series
table(is.na(EFO_data$Series))
dim(EFO_data)

EFO_for_export <- EFO_data %>% 
  mutate(SCALE = 0) %>% # SCALE variable needed in DCS, all 0
...
# Add World values from report (not sum)
World_combined_prep %>% filter(Series %in% EFO_for_export$Series)

EFO_for_export <- rbind(EFO_for_export,
                           World_combined_prep %>% filter(Series %in% EFO_for_export$Series))

# Write complete file as csv
getwd()
write.csv(EFO_for_export, paste0("./Data/", format(Sys.Date(), "%Y%m%d"), 
                                 "GHG_LULUCF_data_for_DCS_combined.csv"))

# Write XSLX file with one sheet per indicator
list_of_indicators_LULUCF <- unique(EFO_for_export$Series)
