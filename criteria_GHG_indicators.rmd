...
The WDI includes `r nrow(WDI_countries)` of which `r sum(WDI_countries$ISO3 %in% PIK_countries)` are included in the PIK dataset. The country coverage for the PIK data is `r round(100 * length(unique(PIK$area..ISO3.)) / nrow(WDI_countries))` percent.
...
#### Span of years
The span of years in the PIK data is `r PIK_coverage_stats$yearlatest - PIK_coverage_stats$yearfirst + 1`.
...
### Country coverage
```{r country_coverage_2, include=FALSE}
# Check which WDI countries available
# Load country WDI list (217 countries)
WDI_countries <- as.data.table(read.csv("Data/wdi_country_list.csv"))
setnames(WDI_countries, c("long_name", "ISO3"))
